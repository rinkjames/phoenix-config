...
onKey(['left', 'j'], hyperShift, () => {
	...
	const frame3 = {width: Math.floor(width / 4), height, x, y};
	let frame = frame2;
	if (objEq(win.frame(), frame2)) {
		frame = frame3;
	}
...
onKey('left', ['ctrl', 'alt', 'cmd'], () => { 
	...
	const {width, height, x, y} = win.screen().flippedVisibleFrame();
	const frame2 = {width: Math.floor((width / 3) * 2), height, x, y};
	const frame3 = {width: Math.floor((width / 4) * 3), height, x, y};
	let frame = frame2;
	if (objEq(win.frame(), frame2)) {
		frame = frame3;
	}
...
onKey('right', ['ctrl', 'alt', 'cmd'], () => {
	...
	const frame2 = {
		...
		height,
		x: x + Math.ceil(width / 3),
		y,
	};
	const frame3 = {
		width: Math.floor((width / 4) * 3),
		height,
		x: x + Math.ceil(width / 4),
		y,
	};
	let frame = frame2;
	if (objEq(win.frame(), frame2)) {
...
onKey('down', ['ctrl', 'shift'], () => {
	const win = Window.focused();
	if (!win) {
		return;
	}

	const wFrame = win.frame()
	const screenSize = win.screen().flippedVisibleFrame()
	const newHeight = Math.max(400, wFrame.height - 20)
	const newWidth = Math.max(600, wFrame.width - 20)
	const newFrame = wFrame
	newFrame.y += Math.round((wFrame.height - newHeight) / 2)
	newFrame.x += Math.round((wFrame.width - newWidth) / 2)
...
onKey('up', ['ctrl', 'shift'], () => {
	...
	const wFrame = win.frame();
	const screenSize = win.screen().flippedVisibleFrame();
	const newHeight = Math.min(screenSize.height, wFrame.height + 20);
	const newWidth = Math.min(screenSize.width, wFrame.width + 20);
	const newFrame = wFrame;
	if (wFrame.y == 0) {
		newFrame.y == 0;
	} else {
		newFrame.y -= Math.round((newHeight - wFrame.height) / 2);
	}
	if (wFrame.x == 0) {
		newFrame.x == 0;
	} else {
		newFrame.x -= Math.round((newWidth - wFrame.width) / 2);
	}
	newFrame.height = newHeight;
	newFrame.width = newWidth;
	win.setFrame(newFrame);
